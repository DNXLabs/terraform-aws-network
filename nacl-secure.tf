locals {
  secure_subnet_ip = split("/",aws_subnet.secure.cidr_block)[0]
  secure_subnet_summary = var.vpc_cidr_summ != "/0" ? cidrhost("${secure_subnet_ip}${var.vpc_cidr_summ}", 0) : aws_subnet.secure.cidr_bloc
}
