locals {
  nat_quantity = local.use_nat_instance ? 1 : var.nat ? var.multi_nat ? length(data.aws_availability_zones.available.names) > var.max_az ? var.max_az : length(data.aws_availability_zones.available.names) : 1 : 0
}
